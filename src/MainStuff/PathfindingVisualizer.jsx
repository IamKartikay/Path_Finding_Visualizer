...
const getNewGridWithWallToggled = (grid, row, col) => {
  const newGrid = grid.slice(); //new array (grid) will be made
  const node = newGrid[row][col];
  if (!node.isStart && !node.isFinish && node.isNode) { //will work for all except starting node and finishing node and works for nodes with isNode === true
    const newNode = {
      ...node,
      isWall: !node.isWall,
    };
...
function getNodesInShortestPathOrder(finishNode) {
  ...
  while (currentNode !== null) {
    nodesInShortestPathOrder.unshift(currentNode); //add to the begining of the array
